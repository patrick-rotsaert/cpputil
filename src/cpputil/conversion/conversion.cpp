...
parsed_time_point parse_time_point(std::string_view in)
{
	...
	{
		CPPUTIL_THROW_EXCEPTION(std::invalid_argument{ "invalid time point format" });
	}
...
parsed_date parse_date(std::string_view in)
{
	...
	if (!std::regex_match(in.begin(), in.end(), matches, re1) || matches.size() != 1 + 3)
	{
		...
		{
			CPPUTIL_THROW_EXCEPTION(std::invalid_argument{ "invalid date format" });
		}
	}
...
parsed_time_of_day parse_time_of_day(std::string_view in)
{
	...
	{
		CPPUTIL_THROW_EXCEPTION(std::invalid_argument{ "invalid time of day format" });
	}
...
void string_to_boost_date(std::string_view in, boost::gregorian::date& out)
{
	...
	if (parsed.year < 0)
	{
		CPPUTIL_THROW_EXCEPTION(std::invalid_argument{ "boost date does not support negative years" });
	}
